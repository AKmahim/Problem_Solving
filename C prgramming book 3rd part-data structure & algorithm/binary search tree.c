...
Node *bst_transplat(Node *root,Node *current_node,Node *new_node)
{
...
Node *bst_delete(Node *root,Node *node)
{
    ...
    if(node->left == NULL){
        ...
        if(smallest_node->parent != node){
            root = bst_transplat(root,smallest_node,smallest_node->right);
            add_right_child(smallest_node,node->right);
        }
        root = bst_transplat(root,node,smallest_node);
